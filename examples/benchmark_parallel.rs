...
fn main() -> Result<()> {
    ...
    println!("Parallel (All):    {:.3} ms ({:.1}x speedup)", 
        par_all_avg, seq_avg / par_all_avg);
    println!("Parallel (Active): {:.3} ms ({:.1}x speedup)", 
        par_active_avg, seq_avg / par_active_avg);
    println!("StatsCollector:    {:.3} ms ({:.1}x speedup)", 
        collector_avg, seq_avg / collector_avg);
    ...
    if interfaces.len() > 4 && par_all_avg < seq_avg * 0.8 {
        ...
        println!("   Expected performance gain: {:.0}%", 
