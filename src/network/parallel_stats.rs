...
pub fn collect_all_stats_parallel(interfaces: &[NetworkInterface]) -> Result<Vec<InterfaceStats>> {
    ...
    let stats: Result<Vec<_>> = interfaces
        ...
        .map(|interface| {
            ...
            collect_interface_stats(interface.index).with_context(|| {
                format!("Failed to collect stats for interface {}", interface.name)
            })
        })
        .collect(); // 결과를 수집
...
pub fn collect_active_stats_parallel(
    interfaces: &[NetworkInterface],
) -> Result<Vec<(usize, InterfaceStats)>> {
    ...
    let stats: Result<Vec<_>> = active_indices
        ...
        .map(|(idx, if_index)| {
            collect_interface_stats(*if_index)
                .map(|stats| (*idx, stats))
                .with_context(|| {
                    format!("Failed to collect stats for interface index {}", if_index)
                })
        })
        .collect();
