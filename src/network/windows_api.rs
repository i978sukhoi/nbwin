...
use winapi::um::iptypes::{GAA_FLAG_INCLUDE_PREFIX, IP_ADAPTER_ADDRESSES};
    ...
    unsafe {
        ...
        if result != ERROR_SUCCESS {
            return Err(anyhow::anyhow!(
                "Failed to get network adapters: error code {}",
                result
            ))
        ...
        while !current_adapter.is_null() {
            ...
            let mut interface = NetworkInterface::new(adapter.u.s().IfIndex, name, description);
...
pub fn get_interface_statistics(interface_index: u32) -> Result<InterfaceStats> {
    unsafe {
        ...
        if result != ERROR_SUCCESS {
            return Err(anyhow::anyhow!(
                "Failed to get interface statistics for index {}: error code {}",
                interface_index,
                result
            ))
